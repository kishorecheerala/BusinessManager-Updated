...
type Action =
  ...
  | { type: 'ADD_PRODUCT'; payload: Product & { reason?: string; referenceId?: string } }
  | { type: 'UPDATE_PRODUCT_STOCK'; payload: { productId: string; change: number; reason?: string; referenceId?: string } }
...
// Helper to create stock history entry
const createStockLog = (productId: string, productName: string, change: number, newQuantity: number, reason: string, referenceId?: string): StockHistoryEntry => {
    return {
...
const appReducer = (state: AppState, action: Action): AppState => {
  ...
  let historyEntry: StockHistoryEntry;
  let historyEntries: StockHistoryEntry[] = [];
  ...
  switch (action.type) {
    ...
    case 'ADD_PRODUCT':
        ...
        if (existingProductIndex >= 0) {
            const existingProd = state.products[existingProductIndex];
            const newQty = existingProd.quantity + newProduct.quantity;
            productsList = state.products.map((p, i) => i === existingProductIndex ? { ...p, quantity: newQty } : p);
            
            // Log Stock History for addition
            historyEntry = createStockLog(newProduct.id, newProduct.name, newProduct.quantity, newQty, action.payload.reason || 'Restock', action.payload.referenceId);
        } else {
            productsList = [...state.products, newProduct];
            // Log Initial Stock
            historyEntry = createStockLog(newProduct.id, newProduct.name, newProduct.quantity, newProduct.quantity, action.payload.reason || 'Initial Stock', action.payload.referenceId);
        }
        
        db.saveCollection('products', productsList);
        db.saveCollection('stock_history', [historyEntry, ...state.stock_history]);
        return { ...state, products: productsList, stock_history: [historyEntry, ...state.stock_history], ...touch };

    case 'UPDATE_PRODUCT_STOCK':
        const targetProd = state.products.find(p => p.id === action.payload.productId);
        if (!targetProd) return state;

        const newStockQty = targetProd.quantity + action.payload.change;
        const updatedStockProducts = state.products.map(p => 
            p.id === action.payload.productId ? { ...p, quantity: newStockQty } : p
        );
        
        historyEntry = createStockLog(
            action.payload.productId, 
            targetProd.name, 
            action.payload.change, 
            newStockQty, 
            action.payload.reason || 'Adjustment', 
        ...
        return { ...state, products: updatedStockProducts, stock_history: [historyEntry, ...state.stock_history], ...touch };
    ...
    case 'ADD_SALE':
        ...
        db.saveCollection('audit_logs', [newLog, ...state.audit_logs]);
        // Note: Stock is updated via separate UPDATE_PRODUCT_STOCK dispatches in component to allow flexibility, 
        // but for robustness, if we moved it here, we'd iterate items and log.
    ...
    case 'UPDATE_SALE':
        ...
        const adjustedProducts = state.products.map(p => {
            if (stockMap[p.id] !== undefined && stockMap[p.id] !== 0) {
                const change = stockMap[p.id];
                const newQ = p.quantity + change;
                
                // Log history for update adjustment
                historyEntries.push(createStockLog(
                    p.id, 
                    p.name, 
                    change, 
                    newQ, 
        ...
        if(historyEntries.length > 0) db.saveCollection('stock_history', [...historyEntries, ...state.stock_history]);
        ...
        return { 
            ...
            stock_history: [...historyEntries, ...state.stock_history],
            ...touch 
        };
    ...
    case 'ADD_PURCHASE':
        ...
        // Stock update is handled via ADD_PRODUCT or UPDATE_PRODUCT_STOCK separately in PurchasePage for finer control, 
        // but ensuring metadata is passed there.
        // However, newPurchase usually comes with items that might update price.
        // This reducer mainly saves the Purchase record itself. The stock effect is done via component dispatch loop or we can move it here.
        // Currently PurchasePage does a loop of ADD_PRODUCT.
    ...
    case 'UPDATE_PURCHASE':
        const { oldPurchase: oldP, updatedPurchase: updatedP } = action.payload;
        const updatedPurchasesList = state.purchases.map(p => p.id === updatedP.id ? updatedP : p);
    ...
    case 'ADD_RETURN':
        ...
        let historyReason = '';
        let historyChangeMultiplier = 1;

        if (newReturn.type === 'CUSTOMER') {
            // Customer returned item -> Increase Stock
            historyReason = 'Customer Return';
            historyChangeMultiplier = 1;
        } else {
            // Returned to Supplier -> Decrease Stock
            historyReason = 'Supplier Return';
            historyChangeMultiplier = -1;
        }
        ...
        stockAdjProducts = state.products.map(p => {
            ...
            if (item) {
                const change = item.quantity * historyChangeMultiplier;
                const newQ = Math.max(0, p.quantity + change);
                historyEntries.push(createStockLog(
                    p.id, 
                    p.name, 
                    change, 
    ...
    case 'CLEANUP_OLD_DATA':
        ...
        const cleanStockHist = state.stock_history.filter(h => new Date(h.timestamp) > thirtyDaysAgo);
        
        db.saveCollection('notifications', cleanNotifs);
        db.saveCollection('audit_logs', cleanLogs);
        db.saveCollection('stock_history', cleanStockHist);
        
        return { ...state, notifications: cleanNotifs, audit_logs: cleanLogs, stock_history: cleanStockHist };
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            const stock_history = await db.getAll('stock_history');
            ...
            dispatch({
                ...
                payload: {
                    ...
                    app_metadata, notifications, audit_logs, stock_history,
