...
export const PurchaseForm: React.FC<PurchaseFormProps> = ({
  ...
  const handleSubmit = () => {
      ...
      const purchaseId = initialData?.id || `PUR-${Date.now()}`;

      // We need to manually dispatch ADD_PRODUCT here for new purchases to update stock properly with reasons
      // The main page component handles the 'ADD_PURCHASE' dispatch, but for consistency, we should ensure stock logic flows correctly.
      // In the previous implementation, PurchasesPage.tsx handled the loop. 
      // Now that we pass `onSubmit`, the parent component (PurchasesPage) calls `handleCompletePurchase`.
      // We need to ensure the parent component passes `reason` and `referenceId` in its dispatch loop.
      // This component just constructs the Purchase object.
