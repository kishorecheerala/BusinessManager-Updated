...
import { Sale, Customer, ProfileData, InvoiceTemplateConfig, CustomFont, Quote, Return, Supplier, Product } from '../types';
...
// ... (Keep existing thermal invoice generator code as is) ...
...
// ... (Keep GenericDocumentData, _generateConfigurablePDF, generateA4InvoicePdf, etc.) ...
...
export const generateProductCatalogPDF = async (
  ...
  includePrice: boolean = true
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(primaryColor);
    const title = profile?.name ? `${profile.name} - Product Catalog` : 'Product Catalog';
    doc.text(title, pageWidth / 2, y, { align: 'center' });
    y += 8;
    ...
    const cardHeight = 80; // Fixed height per product card
    ...
    for (const product of products) {
        ...
        if (product.image) {
            try {
                ...
                // Keep aspect ratio, fit within imageSize box
                const ratio = imgProps.width / imgProps.height;
                let w = imageSize;
                let h = imageSize / ratio;
                if (h > imageSize) {
                    h = imageSize;
                    w = h * ratio;
                }
                // Center image in the top area
                const imgX = x + (cardWidth - w) / 2;
                doc.addImage(product.image, getImageType(product.image), imgX, innerY, w, h);
            } catch(e) {
                // Fallback for broken images
                doc.setFillColor(240);
                doc.rect(x + (cardWidth - imageSize)/2, innerY, imageSize, imageSize, 'F');
                doc.setFontSize(8);
                doc.text('No Image', x + cardWidth/2, innerY + imageSize/2, { align: 'center' });
            }
        } else {
             // Placeholder
        ...
        // Name (Truncate/Wrap)
        const nameLines = doc.splitTextToSize(product.name, cardWidth - (contentPadding * 2));
        doc.text(nameLines[0] + (nameLines.length > 1 ? '...' : ''), x + cardWidth/2, innerY, { align: 'center' });
        innerY += 5;
        ...
        if (product.category) {
            doc.text(product.category, x + cardWidth/2, innerY, { align: 'center' });
            innerY += 4;
        }
        
        doc.text(`Code: ${product.id}`, x + cardWidth/2, innerY, { align: 'center' });
        innerY += 5;
        ...
        if (includePrice) {
            ...
            doc.setFontSize(11);
            doc.setTextColor(primaryColor);
            doc.text(`Rs. ${product.salePrice.toLocaleString('en-IN')}`, x + cardWidth/2, innerY, { align: 'center' });
        }
