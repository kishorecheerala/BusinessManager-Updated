    ...
    for (const storeName of STORE_NAMES) {
        ...
        for (const item of remoteItems) {
            if (item && item.id) {
                ...
                // Profile & Metadata: ALWAYS prefer Cloud if newer or missing (for sync settings)
                // Other Data: Local Wins (Conflict Avoidance)
                if (!localItem) {
                    await store.put(item);
                } else if (storeName === 'profile' || storeName === 'app_metadata') {
                    // Force update settings/profile from cloud if local exists but we are syncing
                    // This ensures business name updates propagate
                    await store.put(item);
                }
            }
        }
