...
import { Customer, Supplier, Product, Sale, Purchase, Return, Notification, ProfileData, AppMetadata, AuditLogEntry, Expense, Quote, CustomFont, Snapshot, StockHistoryEntry } from '../types';
import { AppState } from '../context/AppContext';

const DB_NAME = 'business-manager-db';
const DB_VERSION = 11; // Bumped for Stock History

export type StoreName = 'customers' | 'suppliers' | 'products' | 'sales' | 'purchases' | 'returns' | 'app_metadata' | 'notifications' | 'profile' | 'audit_logs' | 'expenses' | 'quotes' | 'custom_fonts' | 'snapshots' | 'stock_history';
const STORE_NAMES: StoreName[] = ['customers', 'suppliers', 'products', 'sales', 'purchases', 'returns', 'app_metadata', 'notifications', 'profile', 'audit_logs', 'expenses', 'quotes', 'custom_fonts', 'snapshots', 'stock_history'];
...
interface BusinessManagerDB extends DBSchema {
  ...
  stock_history: { key: string; value: StockHistoryEntry; };
}
    ...
    if (!dbPromise) {
        dbPromise = openDB<BusinessManagerDB>(DB_NAME, DB_VERSION, {
            upgrade(db, oldVersion) {
                ...
                // Add index for stock history queries if needed (optional, getAll is fine for small datasets)
                if (db.objectStoreNames.contains('stock_history')) {
                    // db.transaction('stock_history').objectStore('stock_history').createIndex('by_product', 'productId');
                }
            },
        });
    }
...
export async function exportData(): Promise<Omit<AppState, 'toast' | 'selection' | 'pin' | 'googleUser' | 'syncStatus'>> {
    ...
    for (const storeName of STORE_NAMES) {
        ...
        // We include stock_history now as it is core data
